...
description: How to run parallel work in git worktrees without corrupting anything — one tree per stream, branched from the remote tip, closed by rebase and never by force. Use whenever work needs to happen alongside other work in the same repository, or when asked to open, close, list or tidy worktrees.
...
one. Every parallel stream gets its own tree, and the main workdir stays single-threaded.
...
3. `git fetch` and rebase onto the trunk. **On conflict, stop.** Surface the markers; never
   `--skip` or `--abort` on the agent's own judgement.
...
rebase and try again. Force is how their work disappears.
...
wrong checkout. Never use bare stash in a worktree. If the project provides a safe-stash helper,
