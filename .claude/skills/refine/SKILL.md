---
name: refine
description: Turn a vague ask into work an independent agent can pick up cold — map the journey, settle every open question, name the risks, then slice it into vertical tickets with the walking skeleton first. Use when starting anything bigger than a single edit, when a ticket is too thin to hand off, or when asked to refine, spec, or break down work.
...
**Example mapping** (Wynne) — for one behaviour, once the map says which. Take the story, and
...
- **Where to look** — the files, the conventions, the neighbouring code.
- **How it will be verified** — the behaviour to prove, not the implementation to write. Name the
