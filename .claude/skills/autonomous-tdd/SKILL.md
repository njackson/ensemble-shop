---
name: autonomous-tdd
description: Implement a slice against an acceptance test you may not edit, using strict red-green-refactor without pausing for input, and report at the end. Use when handed an outer test and told to make it pass, or when the user wants you to just go and build it.
...
1. Run the outer test. Confirm it fails, and fails for the **right reason** — the behaviour is
...
1. **Write one failing inner test** for the next behaviour on the list, in the domain's words where
...
Files changed: <list>
Spec and step helpers unchanged: yes — the diff contains no test-side edits
