...
1. **Pick a parent.** From the frontier, weighted by how many fixtures it leads on. Early on that is
...
- **Clear the floor.** Run baseline and winner N times each on the validation set and run a
  permutation test with the arm labels flipped within each fixture (the snippet below). Write the
...
If the project has a statistics module, cite it. If not, write these into the project's own
scripts directory once, and cite that; do not keep them in your head.

Picking a parent from the frontier:

```python
import random
# frontier: {candidate_id: {fixture_id: median_score}}
def leaders(frontier):
    best = {}                                   # fixture → (score, [ids])
    for cid, scores in frontier.items():
        for fid, s in scores.items():
            if fid not in best or s > best[fid][0]: best[fid] = (s, [cid])
            elif s == best[fid][0]: best[fid][1].append(cid)
    weight = {}
    for s, ids in best.values():
        for cid in ids: weight[cid] = weight.get(cid, 0) + 1 / len(ids)
...
Clearing the floor — one score per fixture in each arm, same fixture order:
...
def clears_floor(baseline, candidate, alpha=0.05, trials=10_000):
    ...
    for _ in range(trials):
        ...
        for x, y in zip(baseline, candidate):   # flip the arm label within each fixture
            if random.random() < 0.5: x, y = y, x
            b.append(x); c.append(y)
        if abs(mean(c) - mean(b)) >= abs(observed): at_least += 1
    p = at_least / trials
    return observed, p, p < alpha
```

Never shuffle scores freely across fixtures; a hard fixture is hard in both arms, and pretending
otherwise makes chance look quieter than it is.

## Where wins come from, and where they do not

Told by mechanism and date, from one shop's loop, so the rules above survive a hurry.
...
the loop slows to match. No statistics module: the two snippets above, saved into the project. No
