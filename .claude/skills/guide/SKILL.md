...
If the repository is the shop (there is a `README.md` at the root), it has a tag per checkpoint:
...
| 03 | Cut one ticket into three slices; ship the first before writing the second. | Three slices written down, each passing the three questions (demo, ship, observable). The first is merged or deployed before the second exists in the tracker or the log. |
...
| 07 | Break something, push, confirm red, try to merge anyway. | A red run in CI for a deliberate break, and whether merge was blocked. Whether a skip count is pinned. Whether a tripwire job exists. |
...
| 09 | Write a verification report for the most recent change, not-verified section first. | The report exists; "What was not verified" is present and non-empty; each evidence line says what was run. Then dispatch the witness on that same change and compare its report to theirs. |
| 10 | Read the project's context file as a stranger; count claims that cannot be verified. | The count, and one commit that deletes a rule that was no longer true. |
| 11 | Write the topology for something done serially; decide whether one agent would have done. | A file with the shape as code or pseudocode, and a ceiling on rounds, time or cost in it. |
