---
name: parallel-queue
description: Running several agents at once against a backlog without them colliding — worktree per agent, explicit file allow-lists, branch CI as the merge gate, and a queue that refills as each finishes. Use when asked to work several tickets in parallel, fan agents out over a backlog, or keep a queue of agents running.
...
Run the project's preflight if it has one, and stop on a red result. Worktrees consume disk, file
...
- **An explicit file allow-list.** This is the load-bearing instruction: an agent told only its
  goal will helpfully improve neighbouring code, and the diff that arrives is no longer the diff
  that was reviewed. Name the paths it may touch and require it to check its own diff against them
  before committing.
...
collision costs more than the ordering saved — serialise those. Anything cross-cutting (a rename, a
